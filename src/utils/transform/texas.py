...
from utils.transform.constants import TX_IND_FILEPATH, TX_ORG_FILEPATH, TX_TRA_FILEPATH
...
class TexasTransformer(StateTransformer):
    ...
    def clean(self, raw_ind_df : pd.DataFrame, raw_org_df : pd.DataFrame, raw_tra_df: pd.DataFrame) ->tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame]:
        ...
        clean_ind_df["FULL_NAME"] = clean_ind_df.apply(
            lambda row: get_full_name(
                row["FIRST_NAME"], row["LAST_NAME"], row["FULL_NAME"]), axis=1)
        clean_ind_df = clean_ind_df.fillna("Unknown")

        clean_ind_df["LAST_NAME"] = clean_ind_df["LAST_NAME"].str.lower().str.strip()
        clean_ind_df["FIRST_NAME"] = clean_ind_df["FIRST_NAME"].str.lower().str.strip()
        clean_ind_df["FULL_NAME"] = clean_ind_df["FULL_NAME"].str.lower().str.strip()

        return clean_ind_df,raw_org_df,raw_tra_df
    
    def standardize(self,  ind_df : pd.DataFrame, org_df : pd.DataFrame, tra_df: pd.DataFrame) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame]:
        ...
        ind_df["address"] = ind_df.apply(
            lambda row: get_full_name(
                row["ADDRESS_LINE_1"], row["ADDRESS_LINE_2"], row["address"]), axis=1)
        ind_df = ind_df.drop(
            ["Unnamed: 0.1","Unnamed: 0","ADDRESS_LINE_1","ADDRESS_LINE_2","ENTITY_TYPE_SPECIFIC","ORIGINAL_ID"],axis =1)
    ...
    def clean_state(self) ->  tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        clean_ind_df,clean_org_df,clean_tra_df = self.clean(raw_ind_df, raw_org_df, raw_tra_df)
        sd_ind_df, sd_org_df,sd_tra_df = self.standardize(clean_ind_df,clean_org_df,clean_tra_df)
        ind_df,org_df,tra_df = self.create_tables(list[ sd_ind_df, sd_org_df,sd_tra_df])
