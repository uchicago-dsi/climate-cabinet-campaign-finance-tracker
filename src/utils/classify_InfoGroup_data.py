...
def prepare_infogroup_data(
    ...
    business_data_df = pd.read_csv(
        infogroup_csv, sep=",", header=0, chunksize=chunksize
    )
...
def get_InfoGroup_df(
    ...
    Args:
      ...
      chunksize: the number of rows per chunk in the IG dataset (default 10,000 but can be changed for testing purposes)
      num_testing_chunks: number of chunks to iterate through when testing = True
      testing_subset: Boolean - True if code is being tested on only several chunks, False if whole InfoGroup csv should be used
    Returns:
    ...
    InfoGroup_df = prepare_infogroup_data(
        ...
        chunksize=chunksize,
        num_testing_chunks=num_testing_chunks,
